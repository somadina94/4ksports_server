...
export const placeTicket = async (userId, input) => {
    if (!Object.values(TicketType).includes(input.type)) {
    ...
    try {
        ...
        const frozenSelections = input.selections.map((selection) => {
            ...
            if (!event)
                throw new AppError("Selection event missing", 400);
            ...
            const odds = resolveSelectionOdds(event, selection.marketType, selection.pick, selection.line);
            if (odds <= 1) {
        ...
        const totalOdds = Number(calculateTotalOdds(input.type, frozenSelections.map((selection) => selection.odds)).toFixed(4));
        const potentialPayout = calculatePayout(input.stake, totalOdds);
        const { wallet, before, after } = await decrementBalance(userId, input.stake, session);
        ...
        await TicketSelection.insertMany(frozenSelections.map((selection) => ({
            ticketId: ticket._id,
            userId,
