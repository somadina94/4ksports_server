...
const settleTicketFromSelections = (selections, stake) => {
    ...
    if (lostCount > 0)
        return { status: TicketStatus.LOST, payout: 0, wonCount, lostCount, voidCount };
...
export const settleEventSelections = async (eventId) => {
    ...
    for (const selection of pendingSelections) {
        const result = settleSelection(selection.marketType, selection.pick, selection.line, event.scores);
    ...
    for (const ticket of pendingTickets) {
        ...
        try {
            ...
            if (decision.payout > 0) {
                const { wallet, before, after } = await incrementBalance(ticket.userId, decision.payout, session);
                await BalanceTransaction.create([
                    {
                        userId: ticket.userId,
                        balanceWalletId: wallet._id,
                        type: decision.status === TicketStatus.REFUNDED
